...
namespace RFIDP2P3_API.Controllers
{
	...
	public class MasterGIP3Controller : Controller
	{
		...
		public ActionResult<IEnumerable<MasterGIP3>> INQ()
		{
            ...
            using (SqlCommand cmd = new SqlCommand("sp_M_GIP3_Sel", conn))
            {
                ...
                while (sdr.Read())
				{
					ContainerObj.Add(new MasterGIP3
					{
						...
						PartStockMin = sdr["PartStockMon"].ToString(),
		...
		public ActionResult<IEnumerable<MasterGIP2>> UPL(MasterGIP2 paramObj)
		{
			...
			//{
			//	conn.Open();
			//	SqlCommand cmd = new("exec sp_M_GIP3_Ins @PartID, @PlantID, @BuildingID, @BuildingName, @PartNumber, @PartName, @PartDesc, @CycleTimeMin, @CycleTimeSec, @Efficiency, @LineID, @LineName, @QtyperPallet, @QtyProdPerDay, @PartStockMin, @PartStockMax, @PartStockDay, @CycleMax, @LastUpdate, @UserUpdate, @Remarks", conn);
			//	cmd.CommandType = CommandType.StoredProcedure;
			...
			//	cmd.Parameters.Add(new("QtyperPallet", paramObj.QtyperPallet));
			//	cmd.Parameters.Add(new("QtyProdPerDay", paramObj.QtyProdPerDay));
			//	cmd.Parameters.Add(new("PartStockMon", paramObj.PartStockMin));
			//	cmd.Parameters.Add(new("PartStockMax", paramObj.PartStockMax));
			//	cmd.Parameters.Add(new("PartStockDay", paramObj.PartStockDay));
			//	cmd.Parameters.Add(new("CycleMax", paramObj.CycleMax));
		...
		{
            using (SqlConnection conn = new SqlConnection(_configuration))
            using (SqlCommand cmd = new SqlCommand("sp_M_GIP3_Del", conn))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.Add("@Remarks", SqlDbType.VarChar, 100).Direction = ParameterDirection.Output;
